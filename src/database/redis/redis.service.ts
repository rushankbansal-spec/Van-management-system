...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async setVanLocation(
    ...
    try {
      ...
      this.logger.error('Failed to store van location', { vanId, error: error?.message || error });
      throw error;
    }
  }
  ...
  async getVanLocation(vanId: string): Promise<{
    ...
    try {
      ...
      this.logger.error('Failed to get van location', { vanId, error: error?.message || error });
      return null;
    }
  }
    ...
    try {
      ...
      if (responses) {
        for (let i = 0; i < vanIds.length; i++) {
          ...
          const [err, res] = responses[i];
          
          if (!err && res && typeof res === 'string') {
            try {
              results.set(vanId, JSON.parse(res));
            } catch {
      ...
      this.logger.error('Failed to batch get van locations', { error: error?.message || error });
    }
  ...
  async getSchoolVanLocations(schoolId: string): Promise<Map<string, any>> {
    ...
    try {
      ...
      this.logger.error('Failed to scan van locations', { schoolId, error: error?.message || error });
    }
  ...
  async checkRateLimit(
    ...
    try {
      ...
      this.logger.error('Rate limit check failed', { identifier, error: error?.message || error });
  ...
  async getRateLimitStatus(
    ...
    try {
      ...
      this.logger.error('Rate limit status check failed', { identifier, error: error?.message || error });
      return null;
    }
  }
  ...
  async getActiveTrip(vanId: string): Promise<{ tripId: string; schoolId: string; startedAt: number } | null> {
    ...
    try {
      ...
      this.logger.error('Failed to get active trip', { vanId, error: error?.message || error });
    }
